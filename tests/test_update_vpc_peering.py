...
class TestupdateVpcPeering(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.updateVpcPeering = update_vpc_peering.updateVpcPeering(project_id="fakeprojectid", network_name="test-vpc-name", peering_name="gke-ijsadoasoidasod-peer")
        self.updateVpcPeering.return_network = MagicMock(return_value=self.network_mock_data)
        self.updateVpcPeering.update_peering = MagicMock(return_value={"status": True, "msg": "Successfully updated peering gke-ijsadoasoidasod-peer..."})
        self.updateVpcPeering.check_status = MagicMock(return_value=True)
        self.updateVpcPeering.run_peering_update = MagicMock(return_value={"status": True, "msg": "Successfully updated peering gke-ijsadoasoidasod-peer..."})
